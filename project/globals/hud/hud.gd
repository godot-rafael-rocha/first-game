...
@onready var virtual_gamepad: Node2D = $VirtualGamepad
...
@onready var joystick: Node2D = $virtual_gamepad/joystick
...
func _ready():
	virtual_gamepad.visible = _isMobile()
...
func _input(event: InputEvent) -> void:
	if Input.is_action_just_pressed("game_menu"):
		...
		virtual_gamepad.visible = !_show_game_menu
